...
function customCursor() {
    ...
    document.addEventListener('mousemove', t => {
        ...
        if((t.target === mediaCarousel) || (t.target.matches(".custom-cursor-image"))) {
        ...
        closeCursor.style.transform = 'translate(calc(' + curX + 'px - 50%),calc(' + curY + 'px - 50%))';
...
const showMenu = (e) => {
    ...
    const headerDrawerCloser = document.querySelector('#header-menus-transparent--close')
...
const showMenuMobile = (e) => {
    ...
    const thisMenu = e.target.dataset.link
    const thisMenuSelected = document.querySelector(`[data-menu-mobile="${thisMenu}"]`)
...
const closeMenuMobile = (e) => {
 ...
 const openSearchDrawer = (e) => {
    ...
    const offset = { top: rect.top + window.scrollY, bottom: rect.bottom - window.innerHeight, left: rect.left + window.scrollX };
...
Alpine.data('categorieslider', () => ({
    init(element) {
        ...
        swiper.on('activeIndexChange', (sl) => {
                ...
                let slideIndex = sl.activeIndex === 0 ? sl.activeIndex + 1 : sl.activeIndex - 1
